...
# === ИСПРАВЛЕННАЯ ФУНКЦИЯ ПОИСКА (ФИНАЛЬНАЯ ВЕРСИЯ) ===
def search_in_index(query, limit=100):
    """Поиск в индексе - ФИНАЛЬНАЯ ИСПРАВЛЕННАЯ ВЕРСИЯ"""
    ...
    # Для нескольких слов - ищем ТОЛЬКО там, где есть ВСЕ слова в ПОЛНОЙ СТРОКЕ
    else:
        # Получаем все возможные строки, где встречаются наши слова
        placeholders = ','.join(['?'] * len(words))
        
        cursor.execute(f'''
            SELECT i.line_number, i.full_line, i.preview, f.file_name, f.id
            FROM index_words i
            JOIN files f ON i.file_id = f.id
            WHERE i.word IN ({placeholders})
            ORDER BY f.id, i.line_number
        ''', words)
        ...
        for row in cursor.fetchall():
            ...
            # Добавляем найденные слова в строке
            line_lower = row[1].lower()
            for word in words:
                # Проверяем, есть ли слово как отдельное слово (с границами)
                if re.search(r'\b' + re.escape(word) + r'\b', line_lower):
                    lines_dict[key]['words_found'].add(word)
        
        # Оставляем только те строки, где найдены ВСЕ слова
        for line_data in lines_dict.values():
            if len(line_data['words_found']) == len(words):
                key = f"{line_data['file_id']}_{line_data['line']}"
                if key not in seen:
                    seen.add(key)
                    results.append({
                        'file': line_data['file'],
                        'line': line_data['line'],
                        'full_line': line_data['full_line'],
                        'preview': line_data['preview'],
                        'file_id': line_data['file_id']
                    })
...
def start_command(message):
    ...
    try:
        ...
        welcome = f"""
🔍 **PALLADIUM BOT - РАБОТАЕТ!**
{'👑 АДМИНИСТРАТОР' if user_id == ADMIN_ID else '👤 ПОЛЬЗОВАТЕЛЬ'}

✅ Бот успешно запущен!

📌 **ПОИСК:**
/search [текст] - быстрый поиск
...
def admin_delete_command(message):
    if not is_admin(message.from_user.id):
        bot.reply_to(message, "❌ Только для администратора")
        return
    ...
    parts = message.text.split()
    if len(parts) != 2:
        bot.reply_to(message, "❌ Использование: /admin_delete [id]")
        return
    ...
    try:
        ...
        cursor.execute('DELETE FROM index_words WHERE file_id = ?', (file_id,))
        conn.commit()
        conn.close()
        bot.reply_to(message, f"✅ Файл ID {file_id} удален")
    except:
        bot.reply_to(message, "❌ Неверный ID")
...
@bot.message_handler(commands=['identity', 'gi'])
def identity_command(message):
    first_names = ['Александр', 'Дмитрий', 'Максим', 'Анна', 'Мария', 'Елена']
    last_names = ['Иванов', 'Петров', 'Сидоров', 'Иванова', 'Петрова', 'Сидорова']
...
**ФИО:** {surname} {name}
**Телефон:** `{phone}`
**Email:** `{name.lower()}.{surname.lower()}@mail.ru`
...
@bot.message_handler(commands=['card', 'gc'])
def card_command(message):
    bins = ['4276', '4279', '5211', '5489']
    card = f"{random.choice(bins)} {random.randint(1000,9999)} {random.randint(1000,9999)} {random.randint(1000,9999)}"
    
    response = f"""
💳 **ТЕСТОВАЯ КАРТА**

**Номер:** `{card}`
**Срок:** {random.randint(1,12):02d}/{random.randint(25,30)}
...
def stats_command(message):
    ...
    cursor.execute('SELECT COUNT(*) FROM files WHERE is_public = 1')
    public_files = cursor.fetchone()[0]
    
    cursor.execute('SELECT SUM(file_size) FROM files')
    total_size = cursor.fetchone()[0] or 0
    
    cursor.execute('SELECT COUNT(*) FROM index_words')
...
💾 Общий объем: {total_size/(1024**3):.2f} ГБ
🔍 Записей в индексе: {total_index}
    """
...
def ping_command(message):
    ...
    bot.edit_message_text(f"🏓 **Pong!** ({ (end-start)*1000:.0f} мс)", 
...
if __name__ == '__main__':
    print("=" * 50)
    print("🚀 ЗАПУСК PALLADIUM BOT (ПОЛНАЯ ВЕРСИЯ)")
    print("=" * 50)
    print("✅ Быстрый поиск по индексу")
    print("✅ Поиск по нескольким словам (точные совпадения)")
    print("✅ HTML отчеты")
